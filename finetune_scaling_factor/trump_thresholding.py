...
def main():
    ...
    for NUM_SELFISH in range(1, POP_SIZE, 8):
        ...
        num_trumps.append(NUM_SELFISH)
        percent_selfish.append(float(selfish) / POP_SIZE)


    plt.plot(num_trumps, percent_selfish, 'bo')
    plt.xlabel('Number of defectors')
    plt.ylabel('\% selfish members of population')
    plt.ylim(0,1)
    plt.xlim(0, POP_SIZE)
    plt.title('Effect on adding defectors')
    plt.show()
