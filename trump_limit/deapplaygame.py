...
def evaluate(member):
    ...
    if objectives == 2:
        ...
        score1 = float(member[1]) / member[4]
        score2 = min(float(member[3]) / member[4] * 6, 3)
        #score2 = float(member[1] + member[2]) / (2 * member[4])
    if objectives == 3:
...
def exportGenometoCSVavg(filename, population):
    import csv
    with open(filename, 'wb') as csvfile:
        writer = csv.writer(csvfile, delimiter=',', quotechar='|',
                            quoting=csv.QUOTE_MINIMAL)
        ...
        for member in population:
            writer.writerow(member[0] +[float(member[1])/member[4]] + [float(member[2])/member[4]] + [float(member[1] + member[2]) / (2 * member[4])] + [member[4]] +[member[5]])
